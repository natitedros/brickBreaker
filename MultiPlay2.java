...
public class MultiPlay2 extends JPanel implements KeyListener, ActionListener{
    private boolean play = false;
    private Timer time;
    private int delay = 5;
    private int score = 0;//score counter
    private int totalBricks = 21;
    private int paddlePosX2 = 560;
    private int paddlePosX1 = 60;
    private int ballPosX = 120;//position coordinates
    private int ballPosY = 350;
    private int ballDirX = 2;   //speed in the x direction
    private int ballDirY = 7;   //Speed in the y direction
    ...
    public MultiPlay2(Color ballColor, Color paddleColor){
    ...
    public void paint(Graphics g){
        ...
        //paddle 2 rectangle generated 
        g.setColor(paddleColor);
        g.fillRect(paddlePosX2, 690, 75, 10);  
        
        //boundary
        g.setColor(Color.white);
        g.fillRect(498, 685, 4, 115);
        
        //paddle 1 rectangle generated 
        g.setColor(paddleColor);
        g.fillRect(paddlePosX1, 690, 75, 10);  
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(play){
            ...
            if(new Rectangle(ballPosX, ballPosY, 20, 20).intersects(new Rectangle(paddlePosX2, 690, 75, 1)) || new Rectangle(ballPosX, ballPosY, 20, 20).intersects(new Rectangle(paddlePosX1, 690, 75, 1))){
                ballDirY = -ballDirY;
            }
            ...
            if(moveRight2 && (paddlePosX2 < 925)){paddlePosX2 += 10;}
            if(moveRight1 && (paddlePosX1 < 423)){paddlePosX1 += 10;}
            if(moveLeft2 && (paddlePosX2 > 502)){paddlePosX2 -= 10;}
            if(moveLeft1 && (paddlePosX1 > 0)){paddlePosX1 -= 10;}
        }
        repaint();
    }
    ...
    public void keyPressed(KeyEvent ke) {    //implemented from the keylistener interface to start the game if the keys are pressed
        if(ke.getKeyCode() == KeyEvent.VK_RIGHT){
            ...
            if(paddlePosX2 >= 925){
                paddlePosX2 = 925;
                moveRight2 = false;    //making move variables false when paddle reaches the borders
            }
            else{
                moveRight2 = true;
            }
        ...
        if(ke.getKeyCode() == KeyEvent.VK_D){
            ...
            if(paddlePosX1 >= 423){
                paddlePosX1 = 423;
                moveRight1 = false;    //making move variables false when paddle reaches the borders
            }
            else{
                moveRight1 = true;
            }
